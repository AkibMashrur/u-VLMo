...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and self.save_attention:
            ...
            # print(f"Num heads: {self.num_attention_heads}")
            # print(f"Size of hidden state: {hidden_states.size()}")
            # print(f"All heads: {self.all_head_size}")
            # print(f"{attention_probs.size()}")
            flattened_score = attention_probs.flatten()
            # print(f"Flattened cross modal attention scores: {flattened_score.size()}")
            if flattened_score.size()[0] == attention_probs.size()[1] * attention_probs.size()[3]:
                with open("attention.npy", "wb") as f:
                    attention_probs_ = torch.squeeze(attention_probs)
                    np.save(f, attention_probs_.detach().cpu().numpy())
                # print(flattened_score.detach().cpu().numpy())
