...
def plot(imgs, orig_path, question, with_orig=True, row_title=None, **imshow_kwargs):
    ...
    for row_idx, row in enumerate(imgs):
        ...
        for col_idx, img in enumerate(row):
            ...
            if col_idx > 0:
                ax.set_title(f"{question[col_idx - 1]}?", wrap=True)
                ax.title.set_size(8)

    if with_orig:
        axs[0, 0].set(title=f'Original image')
        axs[0, 0].title.set_size(8)

    if row_title is not None:
        for row_idx in range(num_rows):
            axs[row_idx, 0].set(ylabel=row_title[row_idx])
            axs[row_idx, 0].title.set_size(6)
